...
**Multi-function monitors (agreed 2026-09-25).** A `@monitor` or `@protocol` factory may return a sequence of functions instead of one. The group is one configured instance: one instance name, one `path`, one child context, and so one `store_as` namespace that every member shares; that is the point, since accumulating across stages is the trajectory-score case the design argues for and closure state is wrong for it (created once per configuration, shared across samples). At most one member per stage per factory, checked when the factory is called, so `SentinelEvent.stage` identifies which member ran and no sub-names or sub-paths are needed; two functions at one stage are two factories. The decorators validate each member and record the union of their payload types; the runner treats the group as one child and dispatches to the member that accepts the step. Each member still watches exactly one stage, so "One function, one point" stands; what changes is that a factory may emit one function per point when they share state, which is the class-with-a-method-per-stage the design rejected, reached through functions so the static scan and per-function portability verdict still work. `sentinel.md` "One function, one point" and "Instance names" need a paragraph each.
